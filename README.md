...
- 为了训练大家分析时间复杂度的能力，本次作业要求完成一份 **书面复杂度分析报告**，报告提交的截止时间与 OJ 提交的截止时间相同，且在 Code Review 时需要展示你的报告。报告的准确程度会影响你的 Code Review 得分。在报告里，你需要给出你的代码中以下几条指令的时间复杂度，并简要说明你是怎么分析的。指令后方标注的是我们所能接受的最坏复杂度（部分指令存在更优方案），如果你的程序对应部分的复杂度不能满足要求，Code Review 时将被扣除部分分数。
  - 添加队伍 $O(\log n)$
  - 提交题目 $O(\log n)$
  - 刷新榜单 $O(n\log n)$
  - 滚榜 $O(n\log n)$
...
- 本次作业提供部分样例数据供同学们测试使用，数据存放在本仓库 `data` 目录下。注意，这些数据并非 Online Judge 上的测试数据点，本地通过所有数据点并不能保证你能够通过 OJ 上的测试。每位同学有 **一次** 机会从负责本次大作业的助教（林田川 和 周秉霖）那里获得一个 OJ 测试数据点，**同时禁止与同学分享你获得的数据点**。
